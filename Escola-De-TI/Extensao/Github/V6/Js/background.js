...
// Função para obter o histórico e armazenar no Local Storage
function captureAndStoreHistory() {
  chrome.history.search({ text: "", startTime: 0 }, function (historyItems) {
    ...
    console.log("Histórico armazenado no Local Storage:", historyJson);
  });

  // Defina um timeout para chamar a função novamente diariamente
  setTimeout(captureAndStoreHistory, 24 * 60 * 60 * 1000); // 24 horas
}

// Chame a função para iniciar o processo de captura e armazenamento do histórico
